...
# Rules: 17 primary + 10 meta-rules
# ═══════════════════════════════════════════════════════════════════
...
## RULE 6: Turret Decision
**Condition:** Scoring requires aiming at targets that aren't always in front of the robot
**Recommendation:** If the game has scoring targets distributed around the field AND the robot must aim while driving → build a turret. If scoring targets are at fixed field positions that the robot drives up to → skip the turret.
**Confidence:** 80%
**Evidence:** 254 used turrets in 2019, 2022, and 2024 (all games where scoring required aiming at targets from distance). 254 dropped the turret in 2025 because Reef scoring positions were fixed — the robot drove to them. 1690 also skipped the turret in 2025 in favor of a fixed elevator + end effector.
**Exceptions:** Turrets add 3-5 lbs and significant mechanical complexity. If cycle times are the primary differentiator (Rule 11), the turret's weight and complexity may not be worth it. 4414 never uses turrets and wins championships through speed alone.
...
## RULE 10: Game Piece Detection
**Condition:** Game pieces must be collected from the field (not just from human player station)
**Recommendation:** Train a YOLO model for game piece detection. Deploy on Limelight Hailo accelerator.
**Confidence:** 75%
**Evidence:** 1678 used Hailo NN pipeline in 2024. Wave Robotics published YOLOv11n models for 2026 REBUILT. Neural detection is more robust to lighting changes than HSV color thresholding.
**Exceptions:** If game pieces are distinctly colored and the field lighting is controlled, HSV thresholding is simpler and faster to implement. Always have HSV as a fallback to the neural detector. If game pieces are only delivered through a fixed station (no floor pickup), skip game piece detection.
**Spec:** YOLOv8-nano or YOLOv11-nano on Limelight Hailo. 50-200 training images. Roboflow for labeling and augmentation. Single-class detection (game piece only). Confidence threshold 70-80%. See YOLO_TRAINING_GUIDE.md for full deployment workflow.
...
## RULE 12: Weight Management
**Condition:** Any competitive FRC robot
**Recommendation:** Build to the maximum weight limit (125 lbs). Weight saved on mechanisms becomes low-mounted ballast for traction and stability.
**Confidence:** 85%
**Evidence:** 254: "max weight robots like 4414 were just pushing people out of the way." Battery placement is strategic — centered or opposite heaviest mechanism. Bellypan as low as possible (0.5-0.625 inches above ground). CG management discussed in EVERY 254 binder.
**Exceptions:** If the game requires traversing obstacles or bridges, lighter robots have advantages on tipping. This is rare in modern FRC.
**Spec:** Frame: square or near-square for omnidirectional stability. Bellypan: 0.5-0.625 inches above ground. Battery: centered or counterbalancing heaviest mechanism. Remaining weight capacity: lead ballast bolted low inside the frame.
...
## Data Sources (Updated April 2, 2026)
