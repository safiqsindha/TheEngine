...
Security note — Roboflow SDK URL-param auth limitation:
  The official roboflow Python SDK (as of 2026-04-14) authenticates exclusively
  via the ``?api_key=`` URL query parameter for all internal SDK calls such as
  workspace(), project(), and version().  There is no supported Authorization
  header path in the SDK.  This means the key *may* appear in proxy/server logs
  generated by the SDK's own HTTP traffic.

  Mitigations applied here:
  1. probe_api() — which we own fully — uses "Authorization: Bearer <key>"
     header and never puts the key in the URL.
  2. _resolve_api_key() returns a _SafeKey wrapper whose repr() is redacted,
     so accidental log.debug("%s", key) or str(key) never leaks the value.
  3. The key is always loaded from the environment at call-time, never stored
     as a module-level constant.
  4. No log statements in this module reference the raw key value.

  See docs/roboflow_auth_limitation.txt for further detail.
...
class _SafeKey(str):
    """A str subclass whose repr() and str() are redacted.

    This prevents the API key from leaking into log lines such as:
      logger.debug("resolved key=%r", key)
      logger.info("key=%s", key)
    The underlying string value is fully functional for actual API calls.
    """
...
def _resolve_api_key(api_key: Optional[str]) -> _SafeKey:
    """Return api_key if provided, else read ROBOFLOW_API_KEY from env.

    Returns a _SafeKey whose repr/str are redacted to prevent log exposure.
    The real key value is still accessible as a plain string for API calls.

    Raises ValueError with a clear message if neither is available.
    """
    raw = api_key or os.environ.get("ROBOFLOW_API_KEY", "").strip()
    if not raw:
        raise ValueError(
...
def probe_api(api_key: Optional[str] = None) -> dict:
    ...
    """
    key = _resolve_api_key(api_key)
    # Security: send the key via Authorization header, NOT as a URL query param.
    # URL query params are visible in proxy logs, server access logs, and
    # browser history.  The Roboflow REST API accepts Bearer token auth on the
    # workspace-info endpoint even though the Python SDK uses query params
    # internally for its own calls (SDK limitation — see module docstring).
    url = "https://api.roboflow.com/"
    resp = requests.get(url, headers={"Authorization": f"Bearer {str.__str__(key)}"}, timeout=10)
