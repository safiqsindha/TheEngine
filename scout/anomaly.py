...
malfunction, or a breakout/breakdown match. Uses per-team z-score analysis
on a sequence of metric observations.

Algorithm:
  For each team, for each tracked metric (scoring rate, cycle speed, endgame):
    1. Compute team's historical mean and sigma across all their observations
    2. For each observation, compute z = (value − mean) / sigma
    3. Flag entries with |z| > Z_THRESHOLD with structured AnomalyFlag record
...
Usage:
  from anomaly import detect_anomalies, AnomalyFlag
